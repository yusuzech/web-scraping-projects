...
beita_url <- "https://tnmb.org/Forum"
ss_css <- ".uk-nav-sub a"
...
ss_href <- master_page_source %>%
    html_nodes(ss_css) %>%
    html_attr("href")
ss_url <- str_c("https://tnmb.org/",ss_href)
#exclude timeline
...
#upload metadata,thread data and merged tables to google cloud storage
#upload merged tables to bigquery